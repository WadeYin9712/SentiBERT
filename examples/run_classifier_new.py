...
def main():
    ...
    if args.do_train:
        ...
        if False:
            ...
            if task_name == "emoint":
                train_examples = processor.get_train_examples(args.data_dir, args.domain, args.para)
            else:
                train_examples = processor.get_train_examples(args.data_dir, args.para)
            ...
            if task_name == "sstphrase" or task_name == "sst-3":
                ...
                train_features = convert_examples_to_features(
                    train_examples, label_list, args.max_seq_length, tokenizer, output_mode, args.para)
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", disable=args.local_rank not in [-1, 0])):
                ...
                if task_name == "sstphrase" or task_name == "sst-3":
                    ...
                    if (task_name == "sst-2" or task_name == "twitter" or task_name == "emocontext" or task_name == "emoint") and args.para == "sentibert":
                        ...
                        logits, loss = model(input_ids, token_type_ids=segment_ids, attention_mask=input_mask, labels=label_ids)
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        if False:
            ...
            if task_name == "emoint":
                eval_examples = processor.get_dev_examples(args.data_dir, args.domain, args.para)
            else:
                eval_examples = processor.get_dev_examples(args.data_dir, args.para)
            ...
            if task_name == "sstphrase" or task_name == "sst-3":
                ...
                eval_features = convert_examples_to_features(
                        eval_examples, label_list, args.max_seq_length, tokenizer, output_mode, args.para)
        ...
        if task_name == "mnli":
            ...
            all_segment_ids = torch.tensor([f.segment_ids for f in eval_features], dtype=torch.long)
            all_label_ids = torch.tensor([f.label_id for f in eval_features], dtype=torch.long)
            all_graph = torch.tensor([f.graph for f in train_features], dtype=torch.long)
            all_span = torch.tensor([f.span for f in train_features], dtype=torch.long)

            eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_graph, all_label_ids, all_span)
            # Run prediction for full data
            eval_sampler = SequentialSampler(eval_data)
            eval_dataloader = DataLoader(eval_data, sampler=eval_sampler, batch_size=args.eval_batch_size)
